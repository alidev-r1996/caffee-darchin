...
export const TableRow: FC<TableProps> = ({ children, className }) => {
  return <tr className={cn("border-slate-200 dark:border-slate-600 border-b", className)}>{children}</tr>;
};

export const TableColumn: FC<TableProps> = ({ children, className }) => {
  return <td className={cn("py-1 px-6 first:max-w-max text-center whitespace-nowrap mx-auto",className)}>{children}</td>;
};

export const TableTitle: FC<TableProps> = ({ children, className }) => {
  return <th className={cn("py-1 px-6 first:px-2 first:py-3 text-center whitespace-nowrap mx-auto dark:bg-slate-800 bg-slate-100 text-blue-500",className)}>{children}</th>;
};
...
const Table: TableType = ({ children, className }) => {
  return <table className={cn("w-full overflow-x-auto", className)}>{children}</table>;
};
