...
async function getCategory() {
  const res = await fetch("https://www.caffee-darchin.ir/api/categories", {next:{tags:["category"]}});
  return res.json();
}

async function getFoods(params: string) {
  const res = await fetch(`https://www.caffee-darchin.ir/api/foods?title=${params}`, {next:{tags:["food"]}});
  return res.json();
}
