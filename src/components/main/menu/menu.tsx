...
async function getCategory() {
  const res = await fetch("https://cafe-darchin.ir/api/categories", {next:{tags:["category"]}});
...
async function getFoods(params: string) {
  ...
  const res = await fetch(`https://cafe-darchin.ir/api/foods?title=${params}`, {next:{tags:["food"]}});
  return res.json();
}
